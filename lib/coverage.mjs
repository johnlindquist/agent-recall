...
import { MANIFEST, SOURCES, STATE } from "./paths.mjs";
...
export function coverage(db) {
  ...
  if (m?.lastRun?.monitor?.agy > 0) {
    const agyEnabled = fs.existsSync(path.join(STATE, "agy-enabled"));
    gaps.push(agyEnabled
      ? `antigravity: ${m.lastRun.monitor.agy} conversations snapshot-archived (searchable via --raw; not yet indexed)`
      : `antigravity: ${m.lastRun.monitor.agy} conversations NOT yet archived (lane disabled — recall agy-enable)`);
  }
