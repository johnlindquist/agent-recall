...
// Anchored final-generation names only: g<seq>.<known ext>. Never matches
// *.partial, tmp/candidate names, or non-file entries. Capture = sequence.
...
// Committed generations in a dir: regular files matching GEN_RE, numeric order.
const listGens = (d) => {
  try {
    return fs.readdirSync(d, { withFileTypes: true })
      .filter((e) => e.isFile() && GEN_RE.test(e.name))
      .map((e) => e.name)
      .sort((a, b) => Number(a.match(GEN_RE)[1]) - Number(b.match(GEN_RE)[1]) || (a < b ? -1 : a > b ? 1 : 0));
  } catch { return []; }
};
