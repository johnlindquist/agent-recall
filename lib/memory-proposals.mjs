...
import { MEMORY_PROPOSALS, sha } from "./paths.mjs";
...
function readRegularProposal(file) {
  ...
  if (st.size > 64 * 1024) throw new Error("proposal file is too large");
...
function validateStoredProposal(value, id, now) {
  ...
  if (!Number.isFinite(created) || !Number.isFinite(expires) || expires - created !== PROPOSAL_TTL_MS)
  ...
  if (value.items.some((item) => item.scope !== "global")) {
    ...
    if (!p || typeof p !== "object" || Array.isArray(p) ||
        !ownKeysExactly(p, ["top", "base", "dirKey"]) ||
        !path.isAbsolute(p.top) || typeof p.base !== "string" || typeof p.dirKey !== "string")
...
function cleanupAndCountLive(now) {
  ensureProposalDir();
  let names = fs.readdirSync(MEMORY_PROPOSALS).slice(0, MAX_SCAN);
  let live = 0;
  for (const name of names) {
    ...
    try {
      const st = fs.lstatSync(file);
      if (!st.isFile() || st.isSymbolicLink() || st.size > 64 * 1024) { live++; continue; }
...
function boundProject(items, cwd) {
  ...
  const top = fs.realpathSync(pk.top);
  const canonical = projectKey(top);
  return { top: canonical.top, base: canonical.base, dirKey: canonical.dirKey };
}
