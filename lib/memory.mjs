...
function plainProjectTop(cwd) {
  const top = fs.realpathSync(cwd);
  let current = top;
  const currentStat = fs.statSync(top);
  if (!currentStat.isDirectory()) throw new Error("project path is not a directory");
  while (true) {
    const marker = path.join(current, ".git");
    try {
      fs.lstatSync(marker);
      throw new Error(`Git metadata exists but cannot be verified: ${marker}`);
    } catch (error) {
      if (error?.code !== "ENOENT") throw error;
    }
    const parent = path.dirname(current);
    if (parent === current) return top;
    current = parent;
  }
}
...
export function projectKey(cwd = process.cwd()) {
  ...
  let gitDev = null;
  let gitIno = null;
  let gitBirthtimeNs = null;
  try {
    top = gitPath(cwd, "--show-toplevel");
    gitDetected = true;
  } catch (error) {
    // Git's generic not-a-repository text also covers broken metadata. Plain
    // fallback is safe only after a no-follow ancestor scan proves that no
    // .git marker exists anywhere that could own this working directory.
    if (!String(error?.stderr || "").includes("not a git repository")) throw error;
    top = plainProjectTop(cwd);
  }
  if (gitDetected) {
    gitDir = fs.realpathSync(path.resolve(cwd, gitPath(cwd, "--absolute-git-dir")));
    const gitStat = fs.statSync(gitDir, { bigint: true });
    if (!gitStat.isDirectory()) throw new Error("Git directory is not a directory");
    gitDev = String(gitStat.dev);
    gitIno = String(gitStat.ino);
    gitBirthtimeNs = String(gitStat.birthtimeNs);
    kind = "git";
  }
...
const factSources = (cwd) => {
  ...
  return [
    { dir: path.join(MEMORY, "global/facts"), scope: "global", target: null },
    {
...
function decodeUtf8Fatal(bytes, file) {
  try {
    return new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  } catch {
    throw new Error(`memory fact is not valid UTF-8: ${file}`);
  }
}

function validateStoredFact(fact, file) {
  if (!fact.trim()) throw new Error(`empty memory fact body: ${file}`);
  if (fact.length > MAX_FACT) throw new Error(`memory fact body exceeds ${MAX_FACT} chars: ${file}`);
  return fact;
}

function parseFile(file, projectTarget = null) {
  ...
  try {
    ...
    raw = decodeUtf8Fatal(fs.readFileSync(fd), file);
  } finally {
    fs.closeSync(fd);
  }
  const m = raw.match(/^---\n([\s\S]*?)\n---\n?/);
  ...
  if (Object.keys(fm).some((key) => !allowed.has(key)) ||
      ...
      fm.scope !== (projectTarget ? "project" : "global") ||
      (projectTarget ? fm.project_key !== projectTarget.dirKey : fm.project_key !== undefined) ||
...
const readAllFacts = (dir, projectTarget = null) =>
  listFactFiles(dir).map((name) => parseFile(path.join(dir, name), projectTarget));
...
function* activeFacts(cwd, { project = null } = {}) {
  ...
  for (const { dir, scope, target } of selected) {
    const facts = readAllFacts(dir, target);
...
function rememberAtTargetUnlocked(fact, projectTarget = null) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    try {
      ...
      const readFd = fs.openSync(file, fs.constants.O_RDONLY | (fs.constants.O_NOFOLLOW || 0));
      let readback;
      try {
        const readStat = fs.fstatSync(readFd);
        if (!readStat.isFile() || readStat.size !== bodyBytes.length)
          throw new Error("memory fact readback mismatch");
        readback = fs.readFileSync(readFd);
      } finally {
        fs.closeSync(readFd);
      }
      if (!readback.equals(bodyBytes)) throw new Error("memory fact readback mismatch");
...
function findActiveDuplicateAtTarget(fact, projectTarget = null) {
  ...
  for (const dir of factsDirsForTarget(projectTarget)) {
    const facts = readAllFacts(dir, projectTarget);
    for (const p of facts) {
